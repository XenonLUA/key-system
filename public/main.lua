...
local Flags = xenon.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(xenon.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then xenon.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then xenon.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then xenon.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, xenon.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(xenon.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = xenon.Themes[xenon.Save.Theme]

local function AddEle(Name, Func)
	xenon.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = xenon.Elements[Ele](Instance, props, ...)
	return Element
end
...
function xenon:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(xenon.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, xenon.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			xenon.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(xenon.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(xenon.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(xenon.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(xenon.Options, {type = "Section", Name = SectionName, func = Section})
